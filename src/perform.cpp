...
perform::perform()
{
    ...
    //Initialise the maximum tick_off position
    m_max_tick=0;
...
void perform::output_func(void)
{
    while (m_outputing) {
        ...
        while( m_running ){
            ...
            if (dumping) {//TODO sjhdebug: this is how you detect whether a tick is passed - check current_tick
                if ( m_looping && m_playback_mode )
                {
                    if ( current_tick >= get_right_tick() )
                    {
